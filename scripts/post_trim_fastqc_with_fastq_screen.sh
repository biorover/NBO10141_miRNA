...
filenames=$(ls trimmed_reads/*.trimmed.fastq | sort | uniq)
numberfiles=$(echo "$filenames" | sort | uniq |wc -l)
outdir="post_trim_qc/"
...
if [ "$fastqscreen" = true ]; then
	...
	while IFS= read -r a; do ##read list of file IDs one at a time
		fastq_screen --aligner bowtie2 --conf ./fastq_screen.conf --subset 100000 --outdir $outdir/fastq_screen $a
...
if [ "$cleanup" = true ]; then
	...
	mv $outdir/*_fastqc* $outdir/fastqc/ #keeps .zip and .html files from FastQC
